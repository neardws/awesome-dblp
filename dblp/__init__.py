...
DBLP_PUBL_STREAM_URL = DBLP_PUBLICATION_SEARCH_URL + '?q=stream:streams/{type}/{stream_key}:&h={query_number}&format=xml'
...
class Venue(LazyAPIData):
    ...
    def load_data(self):
        ...
        root = etree.fromstring(xml)
        data = {
            'publications':[Publication(publication=publication) for publication in 
                            root.iter('hit')]
        }
        self.data = data
...
class Publication(LazyAPIData):
    ...
    applicable
    """
    def __init__(self, key=None, publication=None):
        self.key = key
    ...
    def load_data(self):
        if self.key is not None:
            ...
            publication = self.publication.xpath('info')[0]
            if publication is None:
                raise ValueError
            data = {
                'type':first_or_none(publication.xpath('type/text()')),
                # 'sub_type':first_or_none(publication.attrib.get('publtype', None)),
                # 'mdate':first_or_none(publication.attrib.get('mdate', None)),
                'authors':first_or_none(publication.xpath('authors/text()')),
                'editors':first_or_none(publication.xpath('editor/text()')),
                'title':first_or_none(publication.xpath('title/text()')),
                'year':int(first_or_none(publication.xpath('year/text()'))),
                'month':first_or_none(publication.xpath('month/text()')),
                'journal':first_or_none(publication.xpath('journal/text()')),
...
def search(type, query_str):
    if type == 'author':
        resp = requests.get(DBLP_AUTHOR_SEARCH_URL, params={'xauthor':query_str})
        root = etree.fromstring(resp.content)
        return [Author(urlpt) for urlpt in root.xpath('/authors/author/@urlpt')]
    elif type == 'venue':
        resp = requests.get(DBLP_VENUE_SEARCH_URL, params={'q':query_str, 'format':'xml'})
        root = etree.fromstring(resp.content)
        return [Venue(hit) for hit in root.iter('hit')]